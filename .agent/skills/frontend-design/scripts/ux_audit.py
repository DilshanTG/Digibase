...
class UXAuditor:
    ...
    def audit_file(self, filepath: str) -> None:
        ...
        # Only detect forms by actual HTML form/input tags, not keywords in variable names
        has_form = bool(re.search(r'<form[\s>]|<input[\s>]|<textarea[\s>]|<select[\s>]', content, re.IGNORECASE))
        complex_elements = len(re.findall(r'<input[\s>]|<select[\s>]|<textarea[\s>]|<option[\s>]', content, re.IGNORECASE))

        # --- 1. PSYCHOLOGY LAWS ---
        # Hick's Law - only count navigation items in nav context
        has_nav_context = bool(re.search(r'<nav|navigation|navbar|sidebar', content, re.IGNORECASE))
        nav_items = len(re.findall(r'<NavLink|<Link|<a\s+href|nav-item', content, re.IGNORECASE))
        # Higher threshold when not in explicit nav context (page content links are OK)
        nav_threshold = 7 if has_nav_context else 12
        if nav_items > nav_threshold:
            if has_nav_context:
                self.issues.append(f"[Hick's Law] {filename}: {nav_items} nav items (Max 7)")
            else:
                self.warnings.append(f"[Hick's Law] {filename}: Many links ({nav_items}). Consider grouping if they compete for attention.")
    ...
    def audit_directory(self, directory: str) -> None:
        ...
        for root, dirs, files in os.walk(directory):
            dirs[:] = [d for d in dirs if d not in {'node_modules', '.git', 'dist', 'build', '.next', 'vendor', 'public'}]
            for file in files:
