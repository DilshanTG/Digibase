...
def check_page(file_path: Path) -> dict:
    ...
    # Detect if this is a layout/template file (has Head component or actual head tag)
    # For React SPAs, only the root HTML file should have these tags
    # Use regex to match <head> or <head  but not <header
    has_head_section = bool(re.search(r'<head[\s>]', content, re.I)) or ('Head>' in content and ('title' in content or 'meta' in content))
    
    # For HTML files, always check. For JSX/TSX, only if they have Head component
    is_html = file_path.suffix.lower() in ['.html', '.htm']
    is_layout = is_html or has_head_section
    
    # 1. Title tag
    has_title = '<title' in content.lower() or ('Head>' in content and 'title' in content.lower())
