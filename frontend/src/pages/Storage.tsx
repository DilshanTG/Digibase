...
export function Storage() {
  ...
  const handleCreateBucket = (e: React.FormEvent) => {
    ...
    // In Digibase, buckets are created on demand.
    // Adding it to local list and selecting it.
    const exists = buckets.find(b => b.name === newBucketName);
    if (!exists) {
      setBuckets([{ name: newBucketName, files_count: 0, total_size: 0, total_size_human: '0 B' }, ...buckets]);
    }
    ...
    <Layout>
      <div className="p-6 lg:p-8 max-w-7xl mx-auto">
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          {/* Sidebar / Buckets */}
          <div className="w-full lg:w-64 flex-shrink-0 animate-slideUp">
            <div className="flex items-center justify-between mb-4">
              <h2 className="font-semibold text-[#ededed]">Buckets</h2>
              <button
                onClick={() => setBucketModalOpen(true)}
                className="p-1 text-[#3ecf8e] hover:bg-[#3ecf8e]/10 rounded transition-all"
                title="Create Bucket"
              >
            ...
            <div className="space-y-1">
              <button
                onClick={() => setSelectedBucket('default')}
                className={`w-full flex items-center justify-between px-3 py-2 rounded-md text-sm transition-all ${selectedBucket === 'default' ? 'bg-[#2a2a2a] text-[#3ecf8e]' : 'text-[#a1a1a1] hover:bg-[#2a2a2a]/50'
                  }`}
              >
              ...
              {buckets.filter(b => b.name !== 'default').map((bucket) => (
                <button
                  key={bucket.name}
                  onClick={() => setSelectedBucket(bucket.name)}
                  className={`w-full flex items-center justify-between px-3 py-2 rounded-md text-sm transition-all ${selectedBucket === bucket.name ? 'bg-[#2a2a2a] text-[#3ecf8e]' : 'text-[#a1a1a1] hover:bg-[#2a2a2a]/50'
                    }`}
                >
                  <div className="flex items-center gap-2">
                    <FolderIcon className="w-4 h-4" />
                  ...
                  <span className="text-[10px] text-[#6b6b6b]">{bucket.files_count}</span>
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex justify-between items-center mb-8 animate-slideUp">
              <div>
                ...
                <p className="text-[#a1a1a1] mt-1 truncate">
                  Bucket: <span className="text-[#3ecf8e] font-mono">{selectedBucket}</span>
                </p>
              </div>
            ...
            <div className="bg-[#2a2a2a] border border-[#3a3a3a] rounded-lg p-4 mb-6 animate-slideUp" style={{ animationDelay: '150ms' }}>
              <div className="flex flex-wrap items-center gap-4">
                ...
                {/* Refresh */}
                <button
                  onClick={() => { fetchFiles(); fetchBuckets(); fetchStats(); }}
            ...
            {isLoading ? (
              ...
              <div className="bg-[#2a2a2a] border border-[#3a3a3a] rounded-lg p-12 text-center animate-slideUp">
                ...
                <h3 className="text-xl font-semibold text-[#ededed] mb-2">No files in this bucket</h3>
                ...
                {files.map((file, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* Actions */}
                      <div className="flex items-center gap-1 mt-2 lg:opacity-0 lg:group-hover:opacity-100 transition-all duration-200">
      ...
      {bucketModalOpen && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 animate-fadeIn">
          <div className="bg-[#2a2a2a] border border-[#3a3a3a] rounded-lg p-6 max-w-sm w-full mx-4 animate-slideUp">
            <h3 className="text-lg font-semibold text-[#ededed] mb-2">Create New Bucket</h3>
            <p className="text-sm text-[#a1a1a1] mb-6">
              Enter a name for your new storage bucket. Names should be lowercase and contain no spaces.
            </p>
            <form onSubmit={handleCreateBucket}>
              <input
                ...
                value={newBucketName}
                onChange={(e) => setNewBucketName(e.target.value.toLowerCase().replace(/\s+/g, '-'))}
                className="w-full px-4 py-2 bg-[#323232] border border-[#3a3a3a] text-[#ededed] rounded-md focus:ring-2 focus:ring-[#3ecf8e] focus:border-transparent outline-none mb-6"
              />
              <div className="flex gap-3">
                <button
                  type="button"
                  onClick={() => setBucketModalOpen(false)}
                  className="flex-1 px-4 py-2 text-[#a1a1a1] bg-[#323232] rounded-md hover:bg-[#3a3a3a]"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  className="flex-1 px-4 py-2 text-black bg-[#3ecf8e] rounded-md hover:bg-[#24b47e] font-medium"
                >
                  Create
                </button>
              </div>
